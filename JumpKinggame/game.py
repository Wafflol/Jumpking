...
class Game:
    ...
    def setMetadata(playerPos, lines, lev):
        ...
        lineBoundaries = lines
        # self.level = lev
    global checkHovering
    def checkHovering(hovering):
        global inAir
        inAir = hovering
    ...
    async def obs(self):
        ...
        sleep(0.001)
        return {"level": self.level, "map": np.asarray(self.map_mat.astype(np.uint8)), "playerPosition": np.asarray(self.playerPosition).astype(np.float32)}
    ...
    async def startGame(self, RenderMode):
        ...
        global page 
        Render = RenderMode
        server_process = subprocess.Popen(['php', '-S', 'localhost:8000', '-t', '/home/wafflol/Downloads/JumpKingAgent/JumpKinggame'],
        ...
        await page.exposeFunction("inAir", checkHovering)
    ...
    async def reward(self):
        ...
        #print(previousYLevel)
        if self.level > prevlevel: # if jumped to new level
            return 5.0
        elif self.level == prevlevel: # still on same level
            #commented is reward for new platform of flat 0.5
            # if self.playerPosition[1] + 200 < previousYLevel:
            #     return 0.5
            # return 0.0
            if self.playerPosition[1] > previousYLevel + 10: # if player fell from last position, return -2 (+10 fixes clipping through ground resulting in negative reward)
                return -2
            elif self.playerPosition[1] == previousYLevel: # if same y level, return -1
                return -1
            return (755.0 - self.playerPosition[1])/755.0*2 # if went up, return max 2 for being at the top, and 0 at the bottom
